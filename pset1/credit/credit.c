...
//THis is the variable in wich the credit card number is stored
long long cc_number;
...
int main(void)
{
    ...
    //Keeps repeating if the while statement is incorrect
    do
    {
        printf("Number: ");
    ...
    //Variable table for checking each indvidual digit
    ...
    //Muktiplies the odd digist by 2
    ...
    //THese following statements are making sure that whenever a digit * 2 is over 10, than store it as the sum of its digits
    if (checks1  >= 10)
    {
    ...
    //These are the sum of the numbers.
    ...
    if (totalsum % 10 == 0 && cc_number > 1000000000000)
    {
        if (cc_number > 10000000000000 && cc_number < 999999999999999)
        {
            if (checknum14 == 3)
            {
                ...
                {
                    printf("AMEX\n");       //The statements ahead link the conditions to the type of card
                }
                else
                {
                    printf("INVALID\n");
        ...
        if (cc_number > 999999999999999)
        {
            ...
            if (checknum15 == 5)
            {
                ...
                else //If what it states before is not true, then
                {
                    printf("INVALID\n");
                }
            }
